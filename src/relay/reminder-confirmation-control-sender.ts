...
export type WhatsAppReminderConfirmationControlSender = (input: {
...
export function createReminderConfirmationControlSender(options: {
	...
	readonly policyStore: ReminderConfirmationControlPolicyStore;
}): WhatsAppReminderConfirmationControlSender {
	return async (input) => {
		...
		if (conversations.length !== 1) throw new Error("reminder control conversation is ambiguous");
		...
		const prepared = options.edgeRuntime.prepareOutbound({
			...
			request: {
				...
				correlationId: `reminder-confirmation:${input.bindingId}:${input.templateId}`,
			},
		});
		...
		try {
			...
			if (!sent) throw new Error("reminder control delivery failed");
		} finally {
