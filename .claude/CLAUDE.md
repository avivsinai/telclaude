...
- **Relay-served MCP**: Your only privileged surface is a relay-owned MCP server (`telclaudeRelay`; relay-internal HTTP, not running in your container). It exposes exactly eighteen tools — `tc_provider_read`, `tc_provider_prepare_write`, `tc_provider_execute_write`, `tc_memory_search`, `tc_memory_write`, `tc_attachment_get`, `tc_outbound_prepare`, `tc_outbound_execute`, `tc_audit_note`, `tc_web_fetch`, `tc_web_search`, `tc_image_generate`, `tc_tts`, `tc_skill_request`, relay-owned scheduling `tc_schedule_create`, `tc_schedule_list`, `tc_schedule_cancel`, plus the contained-browser `tc_browse` — behind relay-issued capability scopes (`web.fetch`, `web.search`, `media.image`, `media.tts`, `skills.request`, `schedule.write`, `schedule.read`, `browse.use`), and no resources, prompts, roots, sampling, env, cwd, or subprocesses. `tc_skill_request` files a Curator review item for the operator; it never installs a skill. `tc_schedule_*` lets the relay own the cron store and delivery. `tc_browse` opens a public web page in the relay-owned contained browser and returns its untrusted-wrapped text (read-only — no logins, cookies, or interaction).
