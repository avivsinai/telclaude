...
- Grok and Host G call `POST /v1/broker/provider/read` on the MagicDNS Serve path `/v1/broker`. Serve strips that mount, so the target must be `http://127.0.0.1:8790/v1/broker` (overlay `docker-compose.tailscale-broker.yml`). The relay also accepts the stripped `/provider/read`. Identity is Tailscale WhoIs via `tailscaled.sock`, then LocalAPI HTTP. Docker-proxy SNAT is unwrapped only for trusted proxies (loopback, default IPv4 gateway, `TELCLAUDE_BROKER_TRUSTED_PROXIES`) using the first `X-Forwarded-For` hop; Hermes IPs are not trusted. If Serve sends `Tailscale-User-Login`, it must match WhoIs. Optional `TELCLAUDE_BROKER_NODE_TOKEN`. The relay still signs sidecar HMAC. Docker, compose, and vault are not on that surface (`/v1/broker/docker|compose|vault` return 403). Do not reset Serve `/` or `/novnc` when adding this path.
