...
- **Served MCP, not a tool allowlist.** Hermes does not get raw tools for privileged work. The relay hosts a relay-internal **live MCP server** that exposes fourteen scoped tools (`tc_provider_read`, `tc_provider_prepare_write`, `tc_provider_execute_write`, `tc_memory_search`, `tc_memory_write`, `tc_attachment_get`, `tc_outbound_prepare`, `tc_outbound_execute`, `tc_audit_note`, `tc_web_fetch`, `tc_web_search`, `tc_image_generate`, `tc_tts`, `tc_skill_request`). The five capability tools additionally require their relay-issued `capabilityScopes` entry (`web.fetch`, `web.search`, `media.image`, `media.tts`, `skills.request`) on the resolved authority; an authority without the scope is denied fail-closed. The server strips any client-supplied authority/connection/provenance fields and resolves the caller's authority from an opaque, peer-bound handle — Hermes cannot name its own scope, memory source, or outbound channels.
...
12. **Hermes exercises only relay-served authority.** The contained runtime acts solely through the fourteen relay-owned MCP tools, bound to an opaque, peer-bound authority handle. Client-supplied authority/connection/provenance fields are stripped; a connection or peer mismatch fails resolution.
