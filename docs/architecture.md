...
Probe evidence is not self-asserted by the runtime. Sensitive probes (served-MCP memory air-gap, skills allowlist, network probes, edge adapters) carry an Ed25519 **runner attestation** signed with the operator relay key — which a contained agent cannot hold — over canonical-JSON digests of the evidence body. Evidence validation checks attestation freshness (artifacts stale after 7 days) and signature validity; archived evidence resolves the trusted relay public key from the checked-in key lock.
