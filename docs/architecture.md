...
- **Served MCP for everything privileged.** All privileged, cross-boundary, durable, or delivery actions go through the relay-internal **live MCP server** (`telclaudeRelay`), never a native tool. It exposes twenty-five scoped tools: `tc_provider_read`, `tc_provider_prepare_write`, `tc_provider_execute_write`, `tc_memory_search`, `tc_memory_write`, `tc_attachment_get`, `tc_outbound_prepare`, `tc_outbound_execute`, `tc_audit_note`, `tc_web_fetch`, `tc_web_search`, `tc_image_generate`, `tc_tts`, `tc_skill_request`, relay-owned scheduling `tc_schedule_create`, `tc_schedule_list`, `tc_schedule_cancel`, the contained-browser `tc_browse` (read-only public web page fetch through the relay-owned browser), the committing browser-interaction tools `tc_browse_act`, `tc_browse_act_prepare`, `tc_browse_act_execute`, and the read-only GitHub repository tools `tc_github_list_repos`, `tc_github_list_refs`, `tc_github_get_tree`, `tc_github_read_file` (relay-native reads through the GitHub App installation token). The capability tools additionally require their relay-issued `capabilityScopes` entry (`web.fetch`, `web.search`, `media.image`, `media.tts`, `skills.request`, `schedule.write`, `schedule.read`, `browse.use`, `browse.act`, `github.read`) on the resolved authority; an authority without the scope is denied fail-closed. Scheduling/reminders go through `tc_schedule` so the relay owns the cron store and delivery — the native `cronjob` toolset is disabled. The server strips any client-supplied authority/connection/provenance fields and resolves the caller's authority from an opaque, peer-bound handle — Hermes cannot name its own scope, memory source, or outbound channels.
...
- **No-fork proof on every pin bump** — `telclaude hermes prove --upstream-clean` proves the pinned Hermes checkout is byte-identical to upstream (`checkout.present/head/expectedRef/pinned/statusClean/diffClean/indexClean`); `hermesCheckoutClean` must be true.
