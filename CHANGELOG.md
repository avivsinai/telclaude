...
- **Heartbeat `/system` copy-paste** — The missing-heartbeat remediation omitted `--private`, so `cron add` rejected the suggested command. It now includes `--private`.
