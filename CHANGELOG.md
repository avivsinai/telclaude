...
- **WhatsApp inbound fallback after non-outbound tools** — The text fallback now skips only when Hermes used `tc_outbound_prepare` / `tc_outbound_execute`. Memory, todo, skills, and other tools no longer swallow a non-empty reply.
- **WhatsApp inbound no-tool replies (#261)** — Successful inbound Hermes text with no outbound tools is sent through authorized `prepareOutbound` plus the outbound dispatcher. Empty turns skip with named content-free outcomes; failures log static codes; completed dispatch logs include tool counts.
