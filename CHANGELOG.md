...
- **WhatsApp inbound no-tool replies** — Successful inbound Hermes text with no outbound tools is sent through authorized `prepareOutbound` plus the outbound dispatcher. Tool-using turns are not double-sent; empty turns skip with named content-free outcomes; failures log static codes; completed dispatch logs include tool counts.
